...
export function getMDXComponents(): MDXComponents {
  return {
    ...
    h1: ({ children, id }) => (
      <h1 id={id} className="scroll-m-20 first:mt-0">
        {children}
      </h1>
    ),
    h2: ({ children, id }) => (
      <h2 id={id} className="scroll-m-20">
        <a href={`#${id}`} className="hover:text-[var(--heading-color)]">
          {children}
        </a>
      </h2>
    ),
    ...
    p: ({ children }) => (
      <p className="leading-[1.7] text-foreground [&:not(:first-child)]:mt-4">
        {children}
      </p>
    ),
    a: ({ href, children }) => {
      ...
      if (isExternal) {
        return (
          <a
            ...
            className="text-foreground hover:text-[var(--heading-color)]"
          >
            {children}
          </a>
        )
      ...
      return (
        <Link href={href || ''} className="text-foreground hover:text-[var(--heading-color)]">
          {children}
        </Link>
      )
    },
