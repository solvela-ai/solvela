...
| `accepts[].scheme` | `"exact"` (direct transfer) or `"escrow"` (on-chain escrow). The [spend-down channel](/docs/concepts/spend-down-channel) is deliberately **not** listed in `accepts[]` — channel-aware clients invoke it directly via the payment header, so the 402 wire is unchanged for SDKs that don't know the scheme. |
