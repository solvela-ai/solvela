...
Channel vouchers are accepted on `/v1/chat/completions`, `/v1/messages`, `/v1/search`, `/v1/solana/price`, and the [A2A paid leg](/docs/concepts/a2a#channel-vouchers-on-a2a) (`message/send`) — one funding primitive, every paid surface. Fund the channel once and an agent can reason, search the web, and price SPL tokens across hundreds of calls without another Solana transaction. Opening and closing a channel are always REST + Solana-signer flows; only the draws differ per surface. See the [Agent Toolbelt](/docs/api/marketplace) for the marketplace tools.
...
The `accepts[]` array in a `402` response lists only `exact` and `escrow` — and the A2A task offer is the same. The channel is **invoked by the client, not offered by the gateway** — channel-aware clients attach the voucher directly after reading the quote (the `payment-signature` header on REST; the `x402.payment.payload` metadata on A2A). This keeps existing SDKs that don't know the scheme fully compatible: the 402 wire is unchanged whether or not the channel is enabled.

## Digest binding per surface

Every voucher signs a `request_digest` binding it to exactly one request. On the REST surfaces that is the SHA-256 of the **raw request bytes**; on A2A — which has no raw body — it is the versioned task binding `SHA-256("solvela-a2a-channel-digest-v1:" + taskId)` (see the [A2A digest contract](/docs/concepts/a2a#channel-vouchers-on-a2a)). The preimage classes are permanently disjoint, so a voucher signed for one surface can never be replayed on another.
