...
<Steps>
  ...
  <Step title="Check for PAYMENT-SIGNATURE header">
    If the `PAYMENT-SIGNATURE` header is absent, the gateway immediately returns **HTTP 402** with a full cost breakdown (base provider cost; no platform fee on the hosted gateway) and a list of accepted payment schemes — both exact USDC-SPL transfer and escrow deposit formats are included.
  </Step>
...
<Steps>
  <Step title="Agent discovery">
    The calling agent fetches `GET /.well-known/agent-card.json` to retrieve the **AgentCard** — a machine-readable description of supported methods, accepted payment schemes, and cost tiers.
  </Step>
...
<Note>
Scoring is pure rule-based and completes in **single-digit microseconds** with zero external calls. There is no ML model involved — the scorer is a deterministic weighted sum over the 15 dimensions.
</Note>
