...
| `platform_fee` | string | Platform fee (default 5% when enabled, 0% on hosted gateway), retained by gateway operator |
| `total` | string | `provider_cost` + `platform_fee` |
| `currency` | string | Always `"USDC"` today |
| `fee_percent` | number | Platform fee percentage (0 on hosted gateway, default 5 on self-hosted, configurable via `SOLVELA_PLATFORM_FEE_PERCENT`) |
