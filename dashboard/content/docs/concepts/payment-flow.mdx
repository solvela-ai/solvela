...
<Info>
`sessionBudget` is a client-side guard only — it does not communicate a limit to the server. The SDK accumulates `cost_breakdown.total` values across all signed payments and stops signing once the sum exceeds the budget.
</Info>
