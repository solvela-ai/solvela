...
<Note>
`solvela-sdk@0.3.0` adds opt-in escrow-program-id pinning
(`EscrowProgramMismatchError` is raised when the gateway advertises an escrow
program that does not match the configured one) and converts post-signing 402s
to `PaymentRejectedError` on both the non-streaming and streaming paths, exactly
like the Go and TypeScript SDKs. It is wire-aligned with the production gateway
at `api.solvela.ai`. Pin to `>=0.3.0` — `solvela-sdk@0.2.0` predates the
