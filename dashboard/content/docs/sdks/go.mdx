...
<Note>
  As of `v0.5.0`, a 402 challenge advertising a payment scheme the SDK does not recognise no longer fails the whole parse — the unknown `accepts[]` entry is skipped (and recorded on the non-wire `UnrecognizedSchemes` field) while scheme *selection* stays fail-closed, so an unknown scheme is never signed. Pin `v0.5.0+` to stay forward-compatible with future gateway schemes.
</Note>
