  ...
  "data": [
    {
      ...
      "pricing": "$0.001/query (no platform fee on hosted gateway)",
      "chains": ["solana"],
      "source": "config",
    ...
    },
    {
      ...
      "pricing": "$0.01/query (no platform fee on hosted gateway)",
      "chains": ["solana"],
      "source": "config",
...
| `pricing` | string | Human-readable pricing label (hosted gateway has no platform fee) |
...
| `price_per_request_usdc` | float \| null | Flat per-call price (no platform fee on hosted gateway; configurable on self-hosted) |
...
<Note>
  `price_per_request_usdc` is the provider cost. `pricing` is the human-readable label. On the hosted gateway, both represent the same amount with no platform fee added. Always pay the `amount` from the 402 challenge, never a number you computed yourself.
</Note>
...
**Price:** `$0.01` per call on the hosted gateway (`10000` atomic USDC; default 5% platform fee when self-hosting).
...
<Warning>
  The price is flat and charged **per call, not per result**. A query that returns zero results costs exactly the same as one that returns ten. Payment is settled before the upstream search runs.
</Warning>
...
**Price:** `$0.001` per call on the hosted gateway (`1000` atomic USDC; default 5% platform fee when self-hosting).
...
- **The flat fee is charged regardless of how many mints resolve.** A 50-mint request where every entry comes back `null` costs the same as one that resolves all 50.
...
Sign a transfer for `accepts[0].amount`, base64-encode the `PaymentPayload`, and resend with the `payment-signature` header. The `cost_breakdown` always separates provider cost from any platform fee (0 on the hosted gateway).
