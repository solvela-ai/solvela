...
| `pricing.fee_percent` | integer | Platform fee percentage (default 5, configurable; currently 0 on hosted gateway) |
...
<Note>
  All prices shown are provider cost only. The hosted gateway at api.solvela.ai has no platform fee, so the amount charged equals the provider cost. The `fee_percent` field in the response reflects the live fee setting (currently 0 on hosted). Self-hosted gateways can enable a platform fee via the `SOLVELA_PLATFORM_FEE_PERCENT` env var.
</Note>
