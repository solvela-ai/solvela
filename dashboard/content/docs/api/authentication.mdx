...
Enterprise organizations can use API keys for traditional header-based authentication. Keys are prefixed with `rcr_key_` and are scoped to an organization.
...
curl https://api.solvela.ai/v1/chat/completions \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer rcr_key_your_key_here" \
  -d '{
    "model": "auto",
    "messages": [{"role": "user", "content": "Hello"}]
  }'
...
See [x402 Overview](/docs/x402-overview) for the full protocol specification.
