...
| `POST` | `/v1/search` | x402-paid web search ($0.01/query + 5% platform fee). Returns normalized `title`/`url`/`snippet`. See [Agent Toolbelt](/docs/api/marketplace#post-v1search) |
| `POST` | `/v1/solana/price` | x402-paid SPL token prices ($0.001/query + 5% platform fee). Up to 50 mints per call; unpriceable mints come back `null`. See [Agent Toolbelt](/docs/api/marketplace#post-v1solanaprice) |
