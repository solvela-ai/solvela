...
| `POST` | `/v1/search` | x402-paid web search ($0.01/query on hosted gateway). Returns normalized `title`/`url`/`snippet`. See [Agent Toolbelt](/docs/api/marketplace#post-v1search) |
| `POST` | `/v1/solana/price` | x402-paid SPL token prices ($0.001/query on hosted gateway). Up to 50 mints per call; unpriceable mints come back `null`. See [Agent Toolbelt](/docs/api/marketplace#post-v1solanaprice) |
| `GET` | `/v1/services` | Discovery: lists every x402 marketplace tool with its endpoint and per-call pricing. No payment required. See [Agent Toolbelt](/docs/api/marketplace) |
