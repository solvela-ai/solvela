...
> Everything *except* the gateway binary is permissively licensed. The protocol, x402, router, and CLI crates, the on-chain escrow program, and every SDK (`@solvela/sdk`, the Python SDK, the Go SDK from `sdks/go/v0.2.0`, the MCP server, the AI SDK provider, the OpenClaw provider) are Apache-2.0 — with an express patent grant. Build on those without contacting us. (Artifacts published before the 2026-05-31 relicense — older crate versions, the PyPI package, and Go `sdks/go/v0.1.0` — still carry MIT metadata until their next release; both licenses are permissive.) See the [licensing table](https://github.com/solvela-ai/solvela#licensing) for the per-component split.
...
- You're embedding any of the libraries (`solvela-protocol`, `solvela-x402`, `solvela-router`, `solvela-cli`) or any SDK. Those are Apache-2.0. Standard notice obligations apply, no payment.
