...
> Everything *except* the gateway binary is permissively licensed. The protocol, x402, router, and CLI crates, the on-chain escrow program, and the SDKs (`@solvela/sdk`, the Python SDK, the MCP server, the AI SDK provider, the OpenClaw provider) are Apache-2.0 — with an express patent grant; the Go SDK is currently MIT. Build on those without contacting us. (Crate versions published before the 2026-05-31 relicense still carry MIT metadata on their registry until their next release; both licenses are permissive.) See the [licensing table](https://github.com/solvela-ai/solvela#licensing) for the per-component split.
