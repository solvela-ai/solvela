...
The gateway's fee-payer and recipient roles now live on a single unified operational wallet `9QGtTUpvLmhggDuBciAeE67MmhECVFYdFLD7xKD4RSno` (set in both `SOLVELA_SOLANA__FEE_PAYER_KEY` and `SOLVELA_SOLANA__RECIPIENT_WALLET` — payer == payee by design), consolidated there during the 2026-05-29→05-31 migration. This is a separate, dedicated key from the upgrade authority above; the old hot-wallet keypair `B7reP7rzz…` was swept to dust and now serves only as the default Solana CLI identity (see the 2026-06-01 forward correction below).

### A2A task IDs — bearer-capability semantics

A2A task IDs (`a2a_` + 128-bit random UUID) act as bearer capabilities: within the task's 600-second Redis TTL, anyone holding the ID can read the task via `tasks/get` — including, for completed paid tasks, the delivered output text and receipt references persisted for client recovery — and can cancel an unpaid quote via `tasks/cancel`. The A2A v0.3 protocol defines no per-task authorization beyond transport, and 128 bits of entropy makes guessing infeasible; `tasks/get` additionally sits behind a dedicated rate limiter (receipts-route parity). Agents integrating over A2A should treat task IDs like receipt IDs: secrets to be stored and transmitted with the same care as the content they unlock.
...
**Fix:** generated a new single-sig keypair off the gateway hot wallet (`EDM9pao5miQdJYfzCtZii9cVn5ZHTBJq84Y9yyqZbsr4`) — stored on the operator workstation (see line 13; not air-gapped or hardware-backed yet), seed phrase backed up out of band — and ran `solana program set-upgrade-authority` to transfer authority off the hot-wallet keypair. The fee-payer key remains `B7reP7rzz…` (no operational change to the gateway), but it no longer carries any upgrade power.

> **Forward correction (2026-06-01):** The fee-payer/recipient role described above no longer lives on `B7reP7rzz…`. During the 2026-05-29→05-31 wallet migration that role was consolidated onto a single unified operational wallet `9QGtTUpvLmhggDuBciAeE67MmhECVFYdFLD7xKD4RSno` (set in both `SOLVELA_SOLANA__RECIPIENT_WALLET` and `SOLVELA_SOLANA__FEE_PAYER_KEY` — payer == payee by design). `B7reP7rzz…` was swept and retired to dust (~0.015 SOL, 0 USDC) and remains only as the default Solana CLI identity; it no longer carries the fee-payer or recipient role. The upgrade authority (`EDM9pao5miQdJYfzCtZii9cVn5ZHTBJq84Y9yyqZbsr4`) is a separate, dedicated key and is unaffected by this consolidation.
