...
- **SDKs** — Python, TypeScript, and a wallet-client (Rust) SDK in separate repos: `solvela-python` (v0.1.0), `solvela-ts` (v0.2.0), `solvela-client` (v0.2.0). The Go SDK was consolidated into the monorepo on 2026-05-12 ([#252](https://github.com/solvela-ai/solvela/pull/252)) and now lives at [`sdks/go/`](./sdks/go) at tag `sdks/go/v0.1.0`; the standalone `solvela-ai/solvela-go` repo is archived with a redirect README. Tagged + GitHub Released 2026-04-29 as the security-hardening release; the Go module is live at the new monorepo path, PyPI/npm/crates.io uploads pending operator credentials.
