...
3. **Trustless Escrow** — Anchor program on Solana mainnet (address: `9neDHouXgEgHZDde5SpZMC7eP7UUGPvfM8v4ULiMfFJA`). Escrow deposits are gated by amount and session duration:
...
- **Mainnet transactions:** [X calls, Y USDC settled]
- **Active wallets:** [N]
...
3. **Performance:** Rust + Axum gateway handles payment verification at scale (tested 400 RPS; competitors top out ~100 RPS in TypeScript).
...
3. **Grant extension** [if applicable — specify] for Phase 2 work (multi-wallet support, advanced routing)
