...
use crate::channels::ChannelRepoError;
...
/// Max acceptable staleness of the RPC-free cached Solana slot the draw checks
/// the voucher expiry against. Past this, [`fetch_cached_slot_bounded`] fails
/// closed (503) rather than measure `expiry_slot - current_slot` against an
/// arbitrarily-old slot during an RPC outage (defence in depth behind the
/// voucher signature + `cumulative ≤ deposited` primary guards).
const CHANNEL_SLOT_MAX_STALENESS: Duration = Duration::from_secs(60);

/// RAII guard for the per-channel draw lock (Decision G).
///
...
impl ChannelDrawLockGuard {
    fn new(state: Arc<AppState>, channel_id: String, token: String) -> Self {
        Self {
            state,
            channel_id,
            token,
        ...
        if let Some(cache) = self.state.cache.as_ref() {
            ...
            .is_err()
            {
                ...
                warn!(
                    ...
                    "chat channel draw lock release timed out — no confirmation the CAS \
